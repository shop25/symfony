...
function _auto_link_urls($text, $href_options = array(), $truncate = false, $truncate_len = 40, $pad = '...')
{
  ...
  $fn = function ($matches) use ($href_options)
  {
  ...
  $fnTruncated = function ($matches) use ($href_options, $truncate_len, $pad)
  {
    if (preg_match("/<a\s/i", $matches[1]))
    {
      return $matches[0];
    ...
    {
      if (strlen($matches[2].$matches[3]) > $truncate_len)
      {
        return $matches[1].
               '<a href="'.($matches[2] == "www." ? "http://www." : $matches[2]).$matches[3].'"'.$href_options.'>'.
               substr($matches[2].$matches[3], 0, $truncate_len).$pad.'</a>'.$matches[4];
      }
      ...
      else
      {
        return $matches[1].
               '<a href="'.($matches[2] == "www." ? "http://www." : $matches[2]).$matches[3].'"'.$href_options.'>'.
               $matches[2].$matches[3].'</a>'.$matches[4];
      }
    }
  };
